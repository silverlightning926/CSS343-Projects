...
Customer::Customer(std::string firstName, std::string lastName, std::string customerID) {
...
void Customer::history() {
    std::cout << "History For Customer " << customerID_ << " - " << firstName_ << " " << lastName_ << std::endl;
    for (const std::pair<Movie, std::string>& pair : customerHistory_) {
        std::cout << "\t" << pair.second << " | " << pair.first.getTitle() << " by " << pair.first.getDirector() << std::endl;
    }
}

std::ostream &operator<<(std::ostream &out, const Customer &customer) {
    out << customer.firstName_ << " " << customer.lastName_ << " - " << customer.customerID_;
    return out;
}
