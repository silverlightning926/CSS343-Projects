...
void InventoryManager::readFile(const std::string &filePath) {
    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, line)) {
            ...
            else if(line[0] == 'I' || line[0] == 'H' || line[0] == 'B' || line[0] == 'R') {
                newCommand(line);
            }
        }
        file.close();
...
InventoryManager::splitString(const std::string &delimiter, const std::string &referenceLine) {
    std::string token;
...
void InventoryManager::newMovie(std::vector<std::string> parsedCommand) {
    ...
    if (token1 == "F") {
        comedyInventory_.emplace_back(std::stoi(parsedCommand[1]), parsedCommand[2], parsedCommand[3],
                                     std::stoi(parsedCommand[4]));
    } else if (token1 == "D") {
        dramaInventory_.emplace_back(std::stoi(parsedCommand[1]), parsedCommand[2], parsedCommand[3],
                                    std::stoi(parsedCommand[4]));
    } else if (token1 == "C") {
        ...
        while (parsedCommand[4].find(delimiter) != std::string::npos) {
            ...
            parsedCommand[4].erase(0, parsedCommand[4].find(delimiter) + delimiter.length());
        }

        parsed_token.push_back(parsedCommand[4]);

        classicInventory_.emplace_back(std::stoi(parsedCommand[1]), parsedCommand[2], parsedCommand[3],
                                      parsed_token[0] + " " + parsed_token[1],
                                      std::stoi(parsed_token[2]), std::stoi(parsed_token[3]));
    }
}
