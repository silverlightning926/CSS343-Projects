...
const std::string &Classic::getMajorActor() const {
    return majorActor;
}
...
Classic::Classic(int stock, const std::string& director, const std::string& title, std::string  majorActor, int releaseMonth, int releaseYear) : releaseYear(releaseYear), releaseMonth(releaseMonth), majorActor(std::move(majorActor)) {
    setStock(stock);
...
std::ostream &operator<<(std::ostream &out, const Classic &classic) {
    out << "Classic (C): " << classic.getTitle() << " by " << classic.getDirector() << " - " << classic.getReleaseMonth() << " / " << classic.getReleaseYear() << " [" << classic.getStock() << "]";
    return out;
}
